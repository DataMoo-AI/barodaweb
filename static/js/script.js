...
function action_trigger() {
    ...
    $.ajax({
        ...
        data: JSON.stringify({ "name": action_name, "policy": "MappingPolicy", "confidence": "0.99" }),
...
function setBotResponse(response) {
    ...
    setTimeout(function() {
        ...
        if (response.length < 1) {
            ...
            var fallbackMsg = "Hi Welcome to Barodaweb!";

            var BotResponse = '<img class="botAvatar" src="barodalogo.JPG"/><p class="botMsg">' + fallbackMsg + '</p><div class="clearfix"></div>';
            ...
            for (i = 0; i < response.length; i++) {
                ...
                if (response[i].hasOwnProperty("text")) {
                    var BotResponse = '<img class="botAvatar" src="barodalogo.JPG"/><p class="botMsg">' + response[i].text + '</p><div class="clearfix"></div>';
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);
                }
                
                
                // check if the response contains links
                if (response[i].hasOwnProperty("link")) {
                    var BotResponse = '<img class="botAvatar" src="barodalogo.JPG"/><p class="botMsg"><a target="_blank" href="' + response[i].link + '">' + response[i].link + '</a></p><div class="clearfix"></div>';
                    $(BotResponse).appendTo(".chats").hide().fadeIn(1000);    
                }
                ...
                if (response[i].hasOwnProperty("attachment")) {
                    ...
                    if (response[i].attachment.type == "video") {
                        ...
                        var BotResponse = '<img class="botAvatar" src="barodalogo.JPG"/><p class="botMsg"><a target="_blank" href="' + response[i].attachment.payload.src + '">' + response[i].attachment.payload.src + '</a></p><div class="clearfix"></div>';
                        $(BotResponse).appendTo(".chats").hide().fadeIn(1000); 
                    }
...
function showBotTyping() {
    ...
    var botTyping = '<img class="botAvatar" id="botAvatar" src="barodalogo.JPG"/><div class="botTyping">' + '<div class="bounce1"></div>' + '<div class="bounce2"></div>' + '<div class="bounce3"></div>' + '</div>'
...
// on click of expand button, get the chart data from global variable & render it to modal
$(document).on("click", "#expand", function() {

    //the parameters are declared globally while we get the charts data from rasa.
